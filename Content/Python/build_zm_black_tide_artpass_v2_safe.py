...
DESTINATION_MAP = "/Game/TopDownShooter/Maps/ZM_BlackTide_ArtPass_V2"
MAIN_MAP = "/Game/TopDownShooter/Maps/MainMap"
SEED = 24071996

ROOT = "/Game/TopDownShooter/Ressources/StarterContent"

# Crash-safe generation settings.
ENABLE_BLUEPRINT_EFFECTS = False
ENABLE_AMBIENT_AUDIO = False
ENABLE_POST_PROCESS = True
CURRENT_STAGE_TAG: Optional[str] = None
ARTPASS_ROOT_FOLDER = "ZM_BlackTide_ArtPass"
...
def configure_actor(
    ...
    if CURRENT_STAGE_TAG is not None and CURRENT_STAGE_TAG not in merged_tags:
        merged_tags.append(CURRENT_STAGE_TAG)

    if merged_tags:
        actor.set_editor_property(
            "tags",
            [unreal.Name(tag) for tag in merged_tags],
        )
...
def spawn_blueprint(
    ...
    if asset_key.startswith("effect_") and not ENABLE_BLUEPRINT_EFFECTS:
        return None
...
def get_current_map_path() -> str:
    """Return the package path of the currently loaded editor world."""
...
def prepare_or_validate_destination() -> bool:
    """Prepare the destination in one run and build it in a second run."""
    if not package_exists(SOURCE_MAP):
        fail(f"Source map not found: {SOURCE_MAP}")

    current_map = get_current_map_path()

    if not package_exists(DESTINATION_MAP):
        if current_map == SOURCE_MAP:
            fail(
                "Open MainMap before preparing the art-pass copy. "
                "The source map must not be loaded."
            )
        ...
        show_message(
            "ZM Black Tide Art Pass - Step 1 complete",
            (
                "La copie sécurisée a été créée.\n\n"
                "Ouvre maintenant :\n"
                "/Game/TopDownShooter/Maps/ZM_BlackTide_ArtPass_V2\n\n"
                "Puis relance exactement le même script. "
                "Le script ne change plus de niveau automatiquement."
            ),
        )
        return False
    ...
    if current_map != DESTINATION_MAP:
        show_message(
            "ZM Black Tide Art Pass - Open the destination map",
            (
                "La map de destination existe déjà.\n\n"
                "Ouvre :\n"
                "/Game/TopDownShooter/Maps/ZM_BlackTide_ArtPass_V2\n\n"
...
def actor_tags(actor: unreal.Actor) -> set[str]:
    """Return actor tags as normalized strings."""
    try:
...
def stage_marker_label(stage_id: str) -> str:
    """Return the stable label used to mark a completed generation stage."""
    return f"ArtPassStageComplete_{stage_id}"


def stage_is_complete(stage_id: str) -> bool:
    """Check whether a stage completion marker exists."""
    expected = stage_marker_label(stage_id)
    return any(
        actor.get_actor_label() == expected
        for actor in unreal.EditorLevelLibrary.get_all_level_actors()
    )


def cleanup_partial_stage(stage_id: str) -> None:
    """Remove actors from an interrupted stage before retrying it."""
    stage_tag = f"ZS_ArtPassStage_{stage_id}"

    for actor in list(unreal.EditorLevelLibrary.get_all_level_actors()):
        if stage_tag in actor_tags(actor):
            unreal.EditorLevelLibrary.destroy_actor(actor)


def mark_stage_complete(stage_id: str) -> None:
    """Create an editor-only marker for a completed stage."""
    spawn_actor(
        unreal.TargetPoint,
        stage_marker_label(stage_id),
        unreal.Vector(0.0, 0.0, -5000.0),
        folder=f"{ARTPASS_ROOT_FOLDER}/GenerationState",
        tags=("ZS_ArtPassStageComplete",),
    )


def save_stage(stage_id: str) -> None:
    """Save after every stage so a crash never loses the complete pass."""
    if not unreal.EditorLevelLibrary.save_current_level():
        fail(f"Unable to save stage: {stage_id}")


def run_stage(stage_id: str, description: str, action) -> None:
    """Run one idempotent generation stage and save it immediately."""
    global CURRENT_STAGE_TAG

    if stage_is_complete(stage_id):
        log(f"Skipping completed stage {stage_id}: {description}")
        return

    log(f"Starting stage {stage_id}: {description}")
    cleanup_partial_stage(stage_id)
    CURRENT_STAGE_TAG = f"ZS_ArtPassStage_{stage_id}"

    try:
        action()
        mark_stage_complete(stage_id)
        save_stage(stage_id)
        log(f"Completed stage {stage_id}: {description}")
    finally:
        CURRENT_STAGE_TAG = None


def stabilize_editor_for_generation() -> None:
    """Disable continuous sky recapture while hundreds of actors are added."""
    for actor in unreal.EditorLevelLibrary.get_all_level_actors():
...
def add_world_shell(library: dict, rng: random.Random) -> None:
    ...
    rings = (
        ("North", -11900.0, 15000.0, 20, 1200.0, 0.0),
        ("South", -11900.0, -21800.0, 20, 1200.0, 0.0),
    )
    ...
    for side, start_x, y, count, spacing, yaw in rings:
        for index in range(count):
            ...
            if window is not None and window_frame is not None and index % 3 == 0:
...
def build_art_pass() -> None:
    """Build the art pass in restartable stages without changing levels."""
    if not prepare_or_validate_destination():
        return

    rng = random.Random(SEED)
    stabilize_editor_for_generation()
    ...
    run_stage(
        "01_WORLD_SHELL",
        "skyline and closed horizon",
        lambda: add_world_shell(library, rng),
    )
    run_stage(
        "02_ARCHITECTURE",
        "roofs, facades and street structure",
        lambda: (add_roofs_and_facades(library), add_street_structure(library)),
    )
    run_stage(
        "03_PLAZA",
        "central plaza and controlled street clutter",
        lambda: add_plaza_and_street_clutter(library, rng),
    )
    run_stage(
        "04_THEATRE",
        "theatre dressing",
        lambda: add_theatre_dressing(library),
    )
    run_stage(
        "05_CLINIC",
        "clinic dressing",
        lambda: add_clinic_dressing(library),
    )
    run_stage(
        "06_MARKET",
        "night market dressing",
        lambda: add_market_dressing(library, rng),
    )
    run_stage(
        "07_POWER",
        "power station dressing",
        lambda: add_power_dressing(library),
    )
    run_stage(
        "08_LAB",
        "laboratory and secret chamber dressing",
        lambda: add_lab_dressing(library),
    )
    run_stage(
        "09_LIGHTING",
        "zone lighting and atmosphere",
        lambda: (add_zone_lighting(), add_atmosphere_and_post_process()),
    )
    run_stage(
        "10_AUDIO",
        "ambient audio",
    ...
    show_message(
        "ZombieSeasons - ZM Black Tide Art Pass V2",
        (
            "La passe immersive sécurisée est terminée.\n\n"
            f"Acteurs totaux : {total_count}\n"
            f"Acteurs art pass : {art_pass_count}\n\n"
            "La génération est sauvegardée après chaque étape. "
            "Les effets Blueprint et l'audio sont désactivés par défaut "
            "pour stabiliser Unreal 5.5."
        ),
    )
...
try:
    ...
    show_message("ZM Black Tide Art Pass V2 - Arrêt", str(error))
except Exception as error:
    unreal.log_error(f"[ZM Black Tide ArtPass] Unexpected error: {error}")
    show_message(
        "ZM Black Tide Art Pass V2 - Erreur inattendue",
        (
            f"{error}\n\n"
