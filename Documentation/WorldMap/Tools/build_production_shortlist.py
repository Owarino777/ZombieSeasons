...
Input:
Documentation/WorldMap/GeneratedEnvironmentCatalog/environment_assets.csv

Output:
Documentation/WorldMap/GeneratedProductionShortlist/
- production_shortlist.csv
- production_shortlist_summary.json

The shortlist is intentionally conservative. It removes obvious technical assets,
ranks remaining meshes by production usefulness, and applies deterministic quotas.
Visual approval still happens later inside Unreal Editor.
"""
...
# These caps keep the first visual-review batch practical while preserving enough
# variety for the four districts and the hub.
...
# Technical/helper assets that should not enter a production placement manifest.
...
def read_catalog(path: Path) -> list[dict[str, str]]:
    """Read the generated environment catalog."""
    if not path.is_file():
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as stream:
        ...
        if missing:
            raise ShortlistError(
                "Catalog is missing required columns: " + ", ".join(sorted(missing))
            )
        return [dict(row) for row in reader]


def hard_reject(row: dict[str, str]) -> tuple[bool, str]:
    """Reject obvious technical meshes using metadata only."""
    ...
    # City Sample vehicle internals are not useful for environmental dressing.
    if row["pack"] == "CitySampleVehicles":
        vehicle_internal_tokens = (
            "wheel",
            "tire",
            "brake",
            "suspension",
        ...
        if any(token in searchable for token in vehicle_internal_tokens):
...
def score_row(row: dict[str, str]) -> tuple[int, list[str]]:
    """Score one candidate according to likely level-production usefulness."""
    searchable = normalize(
        f"{row['asset_name']} {row['package_name']} {row['package_path']}"
    )
    ...
    if row["pack"] == "Scene_UnfinishedBuilding":
        if category in {"ruin_general", "debris_rubble", "construction", "structural"}:
            score += 20
    ...
    # Favor meshes whose names look like primary authored objects rather than data
    # support files. This is deliberately weak because naming conventions vary.
    ...
    # Reward moderate path depth. Extremely deep paths often represent sub-parts,
    # while very shallow paths often represent container/helper assets.
...
def candidate_rows(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], Counter[str]]:
    """Filter and score catalog candidates without loading Unreal assets."""
    rejected = Counter()
    ...
    for row in rows:
        ...
        is_rejected, reason = hard_reject(row)
        if is_rejected:
            rejected[reason] += 1
            continue
...
def apply_category_quotas(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    """Keep the strongest deterministic subset for each production category."""
    ...
    # Preserve minimum representation from each installed pack where possible.
    for pack, minimum in PACK_MINIMUMS.items():
        ...
        additions = [
            row
            for row in rows
            if row["pack"] == pack and row["object_path"] not in selected_paths
        ]
        additions.sort(
...
def write_csv(path: Path, rows: list[dict[str, str]]) -> None:
    """Write the compact shortlist used by the later Unreal visual-review tool."""
    fieldnames = [
        "pack",
        "category",
        "review_state",
...
def write_summary(
    ...
    """Write review statistics and immutable selection policy."""
    payload = {
        ...
        "asset_class_counts": dict(
            sorted(Counter(row["asset_class"] for row in rows).items())
        ),
...
def main() -> None:
    """Generate the deterministic production shortlist."""
    ...
    scored, rejected = candidate_rows(rows)
    shortlist = apply_category_quotas(scored)
